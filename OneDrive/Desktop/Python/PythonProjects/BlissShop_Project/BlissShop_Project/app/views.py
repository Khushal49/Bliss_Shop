...
def wsaloon(request,data=None):
 if data == None:
     wsaloon = Product.objects.filter(category='WW')
 elif data == 'Hair Care' or data == 'Manicure' or data == 'Threading & face wax':
     wsaloon = Product.objects.filter(category='WW').filter(brand=data)
 elif data == 'below':
     wsaloon = Product.objects.filter(category='WW').filter(discounted_price__lt=1200)
 elif data == 'above':
     wsaloon = Product.objects.filter(category='WW').filter(discounted_price__gt=1200)

 return render(request, 'app/wsaloon.html',{'wsaloon':wsaloon})
